...
const ExamReg = () => {
    ...
    useEffect(() => {
        fetch('http://localhost:3001/levelterm')
            .then(res => res.json())
            .then(data => setLevelTermInfo(data))
    }, []);
                ...
                <div className="mx-auto max-w-screen-xl px-4 py-16 sm:px-6 lg:px-8">
                    <div className="rounded-lg bg-white p-8 shadow-lg lg:col-span-3 lg:p-12">
                        ...
                        <form className="space-y-4" onSubmit={handleSubmit}>
                            <legend className='text-center text-3xl font-bold underline'>Exam Registration</legend>
                            <div className="grid gap-4 sm:grid-cols-2">
                                {/* Name */}
                                <div>
                                    <label className='font-medium text-gray-900'>Name:</label>
                                    <select onChange={nameHandle} id="countries" name='name' className="border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:outline-slate-600 block w-full p-2.5 pr-4">
                                        <option defaultValue={'Choose Your Name'}>Choose Your Name</option>
                                        {
                                            teachersName ? (teachersName.map((name, index) => (
                                ...
                                {/* teacher initial */}
                                <div>
                                    <label className='font-medium text-gray-900'>Teacher Initial:</label>
                                    <input type="text" name="initial" value={initial ? initial : 'Select Your Name First'} disabled className="w-full rounded-lg outline outline-1 outline-slate-300 focus:outline-slate-600 p-3 text-sm" />
                                </div>

                                {/* level and term */}
                                <div>
                                    <label className='font-medium text-gray-900'>Level & Term:</label>
                                    <select onChange={lvtHandle} id="countries" name='lvt' className="border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:outline-slate-600 block w-full p-2.5 pr-4">
                                        <option defaultValue={'Choose level & term'}>Choose level & term</option>
                                        {
                                            LevelTerm.map((lvt, index) => (
                                                <option key={index} value={lvt}>{lvt}</option>
                                            ))
                                        }
                                    </select>
                                </div>
                                ...
                                {/* course */}
                                <div>
                                    <label htmlFor="countries" className="block mb-2 text-sm font-medium text-gray-900">Select Course</label>
                                    <select id="countries" name='course' className={`${courses ? '' : 'disabled'}border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:outline-slate-600 block w-full p-2.5`}>
                                        <option>{courses.length > 0 ? 'Choose course' : 'Choose level & term First'}</option>
                                        {
                                ...
                                {/* registered student */}
                                <div>
                                    <label className='font-medium text-gray-900'>Registerd Student:</label>
                                    <input
                                ...
                                <div className={`${students ? 'block' : 'hidden'}`}>
                                    <label className='font-medium text-gray-900'>Total Student:</label>
                                    <input
                                        disabled
                                        name='t_students'
                                        className={`w-full rounded-lg outline outline-1 outline-slate-300 focus:outline-slate-600 p-3 text-sm`}
